    ...
    async getObjectName( page ) {
        try {
            const title = await page.evaluate( () => {
                const elements = document.getElementsByClassName( 'a-size-extra-large' );
    ...
    async getFormats( page ) {
        try {
            const formats = await page.evaluate( () => {
                const elements = document.getElementsByClassName( 'a-unordered-list a-nostyle a-button-list a-horizontal' );
                const buttons = [];

                buttons.push( elements[ 0 ]
                    .getElementsByClassName( 'swatchElement selected resizedSwatchElement' )[ 0 ]
                    .textContent.trim().split( '\n' )[ 0 ].trim() );

                const unselected = elements[ 0 ].getElementsByClassName( 'swatchElement unselected resizedSwatchElement' );
