...
const {
  DuplicateListStatusModel, DuplicateListObjectModel,
} = require('../../../models');
...
const getStatistic = async ({
  ...
  const { result, error } = await DuplicateListStatusModel.find({
    filter: {
      user,
      status: {
        $in: history
...
const getObjectDetails = async ({
  ...
  const { result, error } = await DuplicateListObjectModel.find({
    filter: {
      user,
      importId,
    },
...
const updateImport = async ({
  ...
  const { result, error } = await DuplicateListStatusModel.findOneAndUpdate({
...
const deleteImport = async ({ user, importId }) => {
  ...
  if (error) return { error };
  await DuplicateListObjectModel.deleteMany({
