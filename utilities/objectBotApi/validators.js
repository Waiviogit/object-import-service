...
const createObjectTypeValidate = async ( data ) => {
    ...
    let { objectType } = await ObjectType.getOne( { name: _.get( data, 'objectType' ) } );
...
const createObjectValidate = async ( data ) => {
    ...
    let { wobject } = await Wobj.getOne( { author_permlink: _.get( data, 'permlink' ) } );
